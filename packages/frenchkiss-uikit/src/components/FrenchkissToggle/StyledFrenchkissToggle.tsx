...
const scaleKeyValues = {
  sm: {
    frenchkissSize: "16px", // The size of a frenchkiss (the handle)
    travelDistance: "16px", // How far frenchkisses should travel horizontally
    ...
    butterHeight: "5px", // Height of a butter block on top of frenchkisses
  ...
  md: {
    ...
    toggleHeight: "40px",
    toggleWidth: "72px",
    frenchkissThickness: "2px",
    frenchkissTwoOffset: "-3px",
    frenchkissThreeOffset: "-8px",
    butterTop: "3px",
    butterLeft: "16px",
    butterWidth: "12px",
    butterHeight: "11px",
...
export const FrenchKissStack = styled.div<HandleProps>`
  ...
  .frenchkiss {
    background: #e27c31;
    border-radius: 50%;
    width: ${getScale("frenchkissSize")};
    height: ${getScale("frenchkissSize")};
    position: absolute;
    transition: 0.4s ease;
    top: 2px;
    left: 4px;
    box-shadow: 0 ${getScale("frenchkissThickness")} 0 ${getScale("frenchkissThickness")} #fbbe7c;
  }

  .frenchkiss:nth-child(1) {
    background: ${({ theme }) => theme.frenchkissToggle.handleBackground};
    box-shadow: 0 ${getScale("frenchkissThickness")} 0 ${getScale("frenchkissThickness")}
      ${({ theme }) => theme.frenchkissToggle.handleShadow};
  }

  .frenchkiss:nth-child(2) {
    left: 0;
    top: ${getScale("frenchkissTwoOffset")};
    transform: scale(0);
    transition: 0.2s ease 0.2s;
  }

  .frenchkiss:nth-child(3) {
    top: ${getScale("frenchkissThreeOffset")};
    transform: scale(0);
    transition: 0.2s ease 0.2s;
  }

  .frenchkiss:nth-child(3):before,
  .frenchkiss:nth-child(3):after {
    content: "";
    position: absolute;
    background: #ef8927;
    border-radius: 20px;
...
export const FrenchKissInput = styled.input<InputProps>`
  ...
  &:checked + label .frenchkisses {
    transform: translateX(${getScale("travelDistance")});
  }

  &:checked + label .frenchkiss:nth-child(1) {
    background: #e27c31;
    box-shadow: 0 ${getScale("frenchkissThickness")} 0 ${getScale("frenchkissThickness")} #fbbe7c;
    transition-delay: 0.2s;
  }

  &:checked + label .frenchkiss:nth-child(2) {
    transform: scale(1);
    transition-delay: 0.2s;
  }

  &:checked + label .frenchkiss:nth-child(3) {
    transform: scale(1);
